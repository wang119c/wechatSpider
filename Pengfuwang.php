...
class Duanziwang
{
    ...
    public function run()
    {
        $page = 1;
        while (true) {
            $this->url = sprintf('https://www.pengfue.com/xiaohua_%s.html', $page);
    ...
    public function getQueryData($url)
    {
        ...
        $rules = [
            // 采集文章标题
            'title' => ['.list-item .dl-con dd h1 a', 'text'],
            // 采集文章内容
            'content' => ['.list-item .dl-con dd .content-img', 'text'],
            'url' => ['.list-item .dl-con dd h1 a', 'href'],
        ];
